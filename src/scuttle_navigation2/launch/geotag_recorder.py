...
class GeotagRecorder(Node):
    def __init__(self):
        ...
        # TF listener for getting pose from map->base_link
    ...
    def wait_for_map_frame(self):
        """Wait until map frame exists in TF tree"""
        self.get_logger().info("⏳ Waiting for 'map' frame from SLAM...")
        ...
        while rclpy.ok():
            try:
                ...
                self.get_logger().info("✓ Map frame detected! SLAM is working.\n")
                break
            except (LookupException, ConnectivityException, ExtrapolationException):
                self.get_logger().info("   Still waiting for map frame...", throttle_duration_sec=5.0)
    ...
    def get_pose_from_tf(self):
        """Get current pose from TF (most reliable)"""
        try:
    ...
    def record_waypoint(self):
        ...
        # Try TF first (most reliable)
        pose_data = self.get_pose_from_tf()
